    ...
    <>
        ...
        <div className="container-hc py-14 sm:py-16">
          <Reveal>
            ...
            <div className="flex items-center gap-2 text-xs text-slate mb-4">
              <span className="rounded-full bg-sky text-brand font-semibold px-2.5 py-1">{item.category}</span>
              <span className="inline-flex items-center gap-1"><CalendarDays size={13} /> {item.date}</span>
            </div>
            <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl font-semibold text-navy max-w-3xl leading-tight">
              {item.title}
            </h1>
          </Reveal>
        </div>
      ...
      <section className="section-pad">
        <div className="container-hc grid lg:grid-cols-[1.3fr_0.7fr] gap-14">
          <div>
            <Reveal>
              <div className="relative rounded-3xl overflow-hidden card-shadow aspect-video mb-10">
                <PhotoPlaceholder label={item.title} tone={item.tone} className="h-full w-full" />
              </div>
            </Reveal>
            <Reveal delay={0.1}>
              <p className="text-slate leading-relaxed text-[17px]">{item.body}</p>
            </Reveal>
          </div>
                ...
                {related.map((r) => (
                  <li key={r.slug}>
                    <Link href={`/news/${r.slug}`} className="group flex items-start gap-2 text-sm">
                      ...
                      <span className="text-navy font-medium group-hover:text-brand transition-colors">
                        {r.title}
                      </span>
                    </Link>
                  </li>
