...
export default function AcademicsPage() {
  return (
    <>
      ...
      <section className="section-pad">
        <div className="container-hc grid lg:grid-cols-2 gap-14 items-center mb-16">
          <Reveal>
            ...
            <h2 className="font-display text-3xl sm:text-4xl font-semibold text-[var(--hc-navy)] mt-3 mb-5">
              Learning through play and discovery
            </h2>
            <p className="text-[var(--hc-slate)] leading-relaxed">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {NURSERY_SUBJECTS.map((s, i) => (
            <Reveal key={s.title} delay={i * 0.07}>
              <div className="h-full rounded-2xl border border-[var(--hc-line)] p-6 hover:card-shadow transition-all">
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[var(--hc-sky)] text-[var(--hc-blue)] mb-4">
                  <s.icon size={20} />
                </div>
                <h3 className="font-semibold text-[var(--hc-navy)] mb-1.5">{s.title}</h3>
          ...
          {PRIMARY_SUBJECTS.map((s, i) => (
            <Reveal key={s.title} delay={i * 0.06}>
              <div className="flex items-center gap-4 rounded-2xl bg-white p-5 card-shadow">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[var(--hc-navy)] text-white">
                  <s.icon size={19} />
                </div>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {METHODOLOGY.map((m, i) => (
              <Reveal key={m.title} delay={i * 0.07}>
                <div className="flex gap-5 rounded-2xl border border-[var(--hc-line)] p-7">
                  <span className="font-display text-2xl font-semibold text-[var(--hc-blue)]/30">
                    0{i + 1}
                  </span>
                  <div>
                    <h3 className="font-semibold text-[var(--hc-navy)] mb-1.5">{m.title}</h3>
                    <p className="text-sm text-[var(--hc-slate)] leading-relaxed">{m.text}</p>
                  </div>
                </div>
              </Reveal>
            ))}
      ...
      {/* Exam prep */}
      <section className="section-pad bg-[var(--hc-navy)] text-white">
        <div className="container-hc grid lg:grid-cols-2 gap-14 items-center">
          <Reveal>
            <span className="eyebrow text-[var(--hc-amber)]">Examination Preparation</span>
            ...
            <ul className="space-y-3 text-white/85 text-sm">
              <li className="flex items-center gap-2"><ClipboardCheck size={17} className="text-[var(--hc-amber)]" /> Weekly assessments and progress tracking</li>
              <li className="flex items-center gap-2"><Users size={17} className="text-[var(--hc-amber)]" /> Small revision groups for extra support</li>
              <li className="flex items-center gap-2"><Trophy size={17} className="text-[var(--hc-amber)]" /> Consistent top PSLE pass rates</li>
            </ul>
          </Reveal>
          <Reveal delay={0.1}>
            <div className="relative rounded-3xl overflow-hidden card-shadow aspect-[4/3]">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-12">
            {EXTRA_CURRICULAR.map((e, i) => (
              <Reveal key={e} delay={i * 0.05}>
                <div className="rounded-2xl bg-[var(--hc-sky)] px-6 py-5 text-[var(--hc-navy)] font-medium">
                  {e}
                </div>
              </Reveal>
            ))}
